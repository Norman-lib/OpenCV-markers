...
void initGL(int argc, char * argv[]) {
   ...
   fsRead.open("camera.yml", cv::FileStorage::READ);
   if (!fsRead.isOpened()) {
       cout << "Probleme a la lecture du fichier !" << endl;
   }
   cv::Mat intrinsicReadBack;
   cv::Mat distReadBack;
   fsRead["camera_matrix"] >> intrinsicReadBack;
   fsRead["distortion_coefficients"] >> distReadBack;

   Mat undistortedCurImage;
   ...
   while (!glfwWindowShouldClose(window))
   {
       ...
       undistort(curImg, undistortedCurImage, intrinsicReadBack, distReadBack);

       // Calling ArUco idle
       arucoManager->idle(undistortedCurImage);
       ...
       // Showing images
       imshow(windowNameCapture, undistortedCurImage);
